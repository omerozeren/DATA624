...
```{r message = F}
df_train %>% 
  gather(-PH, -BrandCode, key="Var", value="Value") %>% 
...
```{r message = F}
df_train_cor <- cor(df_train %>% select( -BrandCode), use="complete.obs")
...
"Hyd Pressure1" should be removed from the dataset isce it hold constant values.
